#! /bin/bash

#turn bool_ to 0 to skip specific tests
bool_makefile=0

# const
ITA="\033[3m"
BOLD="\033[1m"
UNDERL="\033[4m"
GREEN="\033[32m"
RED="\033[31m"
YEL="\033[33m"
END="\033[m"
BLU_BG="\033[44m"
YEL_BG="\033[1m"
RED_BG="\033[41;1m"

# exec name
p=scalar

# print intro
echo "------------------------------------"
echo "------------------------------------"
echo -e "${BOLD}CPP module 06 exo 00 (scalar) ${END}"
echo -e "Started at $(date +%R) - $(date +"%d %B %Y")"
echo -e "by $USER on $(uname) os"
echo -e "made by bguillau (@bastienkody)"
echo "------------------------------------"
echo "------------------------------------"

# ------------------------------------------------------------------------------
#				-----		MAKEFILE		-----
# ------------------------------------------------------------------------------
if [[ $bool_makefile == 1 ]] ; then 
echo -e "${YEL_BG}Makefile${END}"

#---------------
# makefile relink
echo -ne "${BLU_BG}Test Makefile${END}\t\t"
make re 1>/dev/null 2> stderrmake.txt
make > stdoutmakebis.txt 2>&1
[[ -s stderrmake.txt ]] && echo -ne "${RED}make wrote on std err${END}" || echo -ne "${GREEN}no make error${END}" 
echo -n " -- "
cat stdoutmakebis.txt | egrep -viq "(nothin|already|date)" && echo -e "${RED}makefile relink?${END}" || echo -e "${GREEN}no relink${END}"
rm -rf stderrmake.txt stdoutmakebis.txt

#-------------------------
# makefile : flags and rule
echo -ne "${BLU_BG}Makefile flags${END}\t\t"
grep -sq -- "-Wall" Makefile && echo -ne "${GREEN}OK (-Wall)${END}" || echo -ne "${RED}-Wall not found${END}"
echo -n " -- "
grep -sq -- "-Wextra" Makefile && echo -ne "${GREEN}OK (-Wextra)${END}" || echo -ne "${RED}-Wextra not found${END}"
echo -n " -- "
grep -sq -- "-Werror" Makefile && echo -ne "${GREEN}OK (-Werror)${END}" || echo -ne "${RED}-Werror not found${END}"
echo -n " -- "
grep -sq -- "-std=c++98" Makefile && echo -ne "${GREEN}OK (std c++98)${END}" || echo -ne "${RED}Flag -std=c++98 not found${END}"
echo ""

else make >/dev/null ; fi

# ------------------------------------------------------------------------------
# 					------- ACCEPTED CHAR -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Accepted char${END}"

echo -ne "${BLU_BG}./${p} \"a\"${END}\t"
./${p} "a"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"A\"${END}\t"
./${p} "A"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \" \" (a space)${END}\t"
./${p} " "
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"*\"${END}\t"
./${p} "*"
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- BAD CHAR -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Bad char${END}"

echo -ne "${BLU_BG}./${p} \"aa\"${END}\t"
./${p} "aa"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"AA\"${END}\t"
./${p} "AA"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \" * \" (space - star - space)${END}\t"
./${p} " * "
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- ACCEPTED INT -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Accepted int${END}"

echo -ne "${BLU_BG}./${p} \"0\"${END}\t"
./${p} "0"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1\"${END}\t"
./${p} "1"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-1\"${END}\t"
./${p} "-1"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"2147483647\"${END}\t"
./${p} "2147483647"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-2147483648\"${END}\t"
./${p} "-2147483648"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"    123\"${END}\t"
./${p} "    123"
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- BAD INT -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Bad int${END}"

echo -ne "${BLU_BG}./${p} \"\" (empty string)${END}\t"
./${p} ""
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"2147483648\"${END}\t"
./${p} "2147483648"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-2147483649\"${END}\t"
./${p} "-2147483649"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"++1\"${END}\t"
./${p} "++1"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"--1\"${END}\t"
./${p} "--1"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1A\"${END}\t"
./${p} "1A"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"23 45\"${END}\t"
./${p} "23 45"
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- ACCEPTED FLOAT -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Accepted float${END}"

echo -ne "${BLU_BG}./${p} \"1.f\"${END}\t"
./${p} "1.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \".1f\"${END}\t"
./${p} ".1f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"0.1f\"${END}\t"
./${p} "0.1f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"0000.001f\"${END}\t"
./${p} "0000.001f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-129.f\"${END}\t"
./${p} "-129.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-128.f\"${END}\t"
./${p} "-128.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"127.f\"${END}\t"
./${p} "127.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"128.f\"${END}\t"
./${p} "128.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"  123.f\"${END}\t"
./${p} "  123.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"134217729.f\"${END}\t"
./${p} "134217729.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"2147483647.f\"${END}\t"
./${p} "2147483647.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-2147483648.f\"${END}\t"
./${p} "-2147483648.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"2147483648.f\"${END}\t"
./${p} "2147483648.f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-2147483649.f\"${END}\t"
./${p} "-2147483649.f"

echo -ne "${BLU_BG}./${p} \"+inff\"${END}\t"
./${p} "+inff"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-inff\"${END}\t"
./${p} "-inff"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"nanf\"${END}\t"
./${p} "nanf"
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- BAD FLOAT -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Bad float${END}"

echo -ne "${BLU_BG}./${p} \".f\"${END}\t"
./${p} ".f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"..f\"${END}\t"
./${p} "..f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1..0f\"${END}\t"
./${p} "1..0f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1ff\"${END}\t"
./${p} "1ff"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"f1f\"${END}\t"
./${p} "f1f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"10f\"${END}\t"
./${p} "10f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1  0f\"${END}\t"
./${p} "1  0f"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"inff\"${END}\t"
./${p} "inff"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"+nanf\"${END}\t"
./${p} "+nanf"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-nanf\"${END}\t"
./${p} "-nanf"
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- ACCEPTED DOUBLE -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Accepted double${END}"

echo -ne "${BLU_BG}./${p} \"1.\"${END}\t"
./${p} "1."
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \".1\"${END}\t"
./${p} ".1"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"0.1\"${END}\t"
./${p} "0.1"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"0000.001\"${END}\t"
./${p} "0000.001"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"  123\"${END}\t"
./${p} "  123"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"2147483647.\"${END}\t"
./${p} "2147483647."
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-2147483648.\"${END}\t"
./${p} "-2147483648."
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"2147483648.\"${END}\t"
./${p} "2147483648."
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-2147483649.\"${END}\t"
./${p} "-2147483649."

echo -ne "${BLU_BG}./${p} \"+inf\"${END}\t"
./${p} "+inf"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-inf\"${END}\t"
./${p} "-inf"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"nan\"${END}\t"
./${p} "nan"
echo "----------------------------------------------------------------"

# ------------------------------------------------------------------------------
# 					------- BAD DOUBLE -------
# ------------------------------------------------------------------------------

echo -e "${YEL_BG}Bad double${END}"

echo -ne "${BLU_BG}./${p} \".0.\"${END}\t"
./${p} ".0."
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"..10\"${END}\t"
./${p} "..10"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1..0\"${END}\t"
./${p} "1..0"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"1. 0\"${END}\t"
./${p} "1. 0"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"...\"${END}\t"
./${p} "..."
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"inf\"${END}\t"
./${p} "inf"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"+nan\"${END}\t"
./${p} "+nan"
echo "----------------------------------------------------------------"

echo -ne "${BLU_BG}./${p} \"-nan\"${END}\t"
./${p} "-nan"
echo "----------------------------------------------------------------"


# ------------------------------------------------------------------------------
#				-----		OUTRO		-----
# ------------------------------------------------------------------------------
make fclean &>/dev/null

echo "###############################################################"
echo "END"
echo "###############################################################"
