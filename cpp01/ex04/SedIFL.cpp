...
bool	SedIFL::init(const std::string Filename, const std::string ReplaceExtension)
{
	...
	_outfile.open(outfileNaming(Filename, ReplaceExtension).c_str(), std::ios::out | std::ios::trunc);
	if (_outfile.fail())
...
{
	std::string	inString = std::string(_infileSize, ' ');

	if (_infileSize > 0 && _infile.read(const_cast<char*>(inString.c_str()), _infileSize))
